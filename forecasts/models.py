...
UKRiskGridRun:         Metadata for a UK-wide risk grid generation.
UKRiskGridPoint:       Individual grid point hourly data.
CachedContourImage:    Pre-rendered contour map PNGs for instant animation.
"""
...
class CachedContourImage(models.Model):
    """
    Pre-rendered contour map PNG for instant map animation.
    Generated by the generate_contour_cache management command
    after each risk_grid run completes.
    """

    run = models.ForeignKey(
        UKRiskGridRun,
        on_delete=models.CASCADE,
        related_name="cached_images",
    )
    timestamp = models.DateTimeField()
    ...
    image_data = models.BinaryField(
        help_text="PNG image bytes (pre-rendered contour map)"
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["run", "variable", "timestamp"]),
        ]

    def __str__(self):
        return f"Contour {self.variable} @ {self.timestamp:%Y-%m-%d %H:%M}"
